...
NB : la fonction utilisée ici est `tbl_summary` du package `gtsummary` permet de réaliser des tableaux de synthèse avec des tests statistiques intégrés, ce qui facilite la présentation des résultats.
...
```{r}
#| label: nb patients par visite et groupe
#| echo: false
#| eval: true
#| message: false
#| results : asis
...
L'objectif est donc de déterminer si un des groupes répond mieux au traitement que l'autre, en comparant l'évolution des scores bruts entre J0 et J56 dans chaque groupe.
...
# LOCF "simple" (style de ton 2e exemple) :
# 1) passer en wide pour matérialiser les visites absentes en NA
# 2) repasser en long (146*8 lignes)
# 3) LOCF par patient
library(zoo)

lhdrs <- reshape2::dcast(
    hdrs_groupe,
    NUMERO + GROUPE ~ VISIT,
    value.var = "score"
)
...
hdrs_groupe_locf <- hdrs_groupe_locf[order(hdrs_groupe_locf$NUMERO, hdrs_groupe_locf$VISIT), ]

# LOCF avec zoo::na.locf, patient par patient (pas de tidyverse / pas de apply)
hdrs_groupe_locf$score_locf <- hdrs_groupe_locf$score
ids <- unique(hdrs_groupe_locf$NUMERO)
for (k in seq_along(ids)) {
    idx <- hdrs_groupe_locf$NUMERO == ids[k]
    hdrs_groupe_locf$score_locf[idx] <- zoo::na.locf(hdrs_groupe_locf$score[idx], na.rm = FALSE)
}
