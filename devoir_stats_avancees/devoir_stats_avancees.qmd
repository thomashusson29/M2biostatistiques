...
"On répond aussi parfois à cette question en considérant le critère binaire censuré « réponse au traitement » défini par une chute de 50% à l’échelle de Hamilton par
rapport à J0. En utilisant ce nouveau critère, répondez donc de nouveau à la question 2."
:::
...
# score et visite de la dernière observation disponible
visits_matrix <- as.matrix(hdrs_wide_reponse[, visits])
last_idx <- apply(visits_matrix, 1, function(x) {
    idx <- which(!is.na(x))
    if (length(idx) == 0) {
        return(NA_integer_)
    }
...
hdrs_wide_reponse$derniere_visite <- NA_character_
hdrs_wide_reponse$score_dernier <- NA_real_
valid <- !is.na(last_idx)
...
# réponse binaire: chute de 50% ou plus à la dernière visite observée
...
# données au format long avec la réponse par patient
