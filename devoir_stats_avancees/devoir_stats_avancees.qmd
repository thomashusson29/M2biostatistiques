...
Test de log-rank pour comparer les courbes de survie de la réponse entre les deux groupes : 
...
Le test de log-rank montre une différence significative entre les deux groupes de traitement (p < 0.01), indiquant que la probabilité de réponse au traitement diffère entre les groupes.

Mais le test de log-rank permet seulement une comparaison globale des courbes de survie entre les groupes.

Pour évaluer l'effet du groupe de traitement sur le temps jusqu'à la réponse, on peut ajuster un modèle de Cox.
...
affichage dans un tableau des résultats du modèle de Cox
...
Pour vérifier l'hypothèse des risques proportionnels, on peut utiliser la fonction `cox.zph` :
