...
# La p-value est la proportion de ces cas
p_value <- n_inf / length(res$t)
p_value
```
...
cat("La p-value unilatérale pour le test kappa < 0.2 est de ", p_value, "\n")
```
...
# Calculer la p-value bootstrap (bilatérale) et l'intervalle de confiance BCa
p_val_bootstrap <- 2 * sum(resboot$t[,2] <= 0) / resboot$R
ci_bca <- boot.ci(resboot, index = 2, type = "bca")

# Mettre en forme la sortie en une seule chaîne de caractères pour un affichage propre
output_string <- paste0(
  sprintf("L'intervalle de confiance à 95%% pour le coefficient de la variable scz.cons est [%.3f, %.3f].\n", ci_bca$bca[4], ci_bca$bca[5]),
  sprintf("La p-value bilatérale pour le test du coefficient de la variable scz.cons est de %.4f.\n", p_val_bootstrap)
)
