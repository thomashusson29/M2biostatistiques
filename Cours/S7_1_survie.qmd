...
NB : toutes les variables correspondant à une durée ne sont pas nécessairement censurées !
...
On aimerait que le délai $T_i$ soit le délai jusqu'à l'événement d'intérêt pour chaque individu $i$.
...
En l'absence de censure (c'est-à-dire quand le délai de survenue de l'événement est connu pour tous les individus), l'approche non paramétrique consiste à **remplacer la probabilité théorique par une simple proportion** basée sur les données observées.

L'estimateur empirique de la fonction de répartition $\hat{F}(t)$ représente la **proportion d’individus** dans la population étudiée qui ont présenté l’événement d'intérêt avant ou à un moment donné $t$.

-   **Calcul :** Cette proportion est calculée en sommant les indicateurs (qui valent 1 si l'événement $T_i$ a eu lieu avant $t$, et 0 sinon) pour tous les individus, puis en divisant par le nombre total d'individus $n$,.

-   **Propriété :** Cet estimateur est une **fonction en escalier, monotone, croissante de 0 à 1**.
...
L'estimateur empirique de la fonction de survie ($\hat{S}(t)$) est le complément direct de $\hat{F}(t)$, soit $\hat{S}(t) = 1 - \hat{F}(t)$.

-   **Signification :** Il représente la **proportion d’individus n'ayant pas présenté l'événement avant** $t$, ou ayant « survécu au-delà de $t$ » dans la population étudiée. Il est calculé comme le nombre d'individus survivant au-delà de $t$, divisé par le nombre total d'individus.

-   **Propriété :** C'est une **fonction en escalier, monotone, décroissante de 1 à 0**.
