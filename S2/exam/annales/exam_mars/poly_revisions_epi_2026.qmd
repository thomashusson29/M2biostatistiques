...
En pratique, on peut résumer leur démarche ainsi :

1. ils divisent les données en `10` plis ;
2. ils laissent un pli de côté comme jeu de test externe ;
3. sur les `9` autres plis, ils refont une validation croisée pour voir quels base learners prédisent le mieux ;
4. cette étape interne sert à construire le `Super Learner`, c'est-à-dire à choisir les poids donnés aux différents algorithmes ;
5. une fois la combinaison fixée, ils l'appliquent au pli externe laissé de côté ;
6. ils répètent ce processus en faisant tourner les `10` plis.
