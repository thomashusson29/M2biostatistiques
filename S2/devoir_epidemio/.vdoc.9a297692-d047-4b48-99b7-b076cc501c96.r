...
or_180d_nonajuste <- epitools::oddsratio(
  x = table(df_avant_imputation$rhc, df$death_180d),
  method = "wald",
  conf.level = 0.95
)
...
surv_object_brut <- Surv(time = df$survival_time_180d, event = df$event_180d)
# ajustement du modèle de survie de Kaplan-Meier
km_fit <- survfit(surv_object_brut ~ rhc, data = df)
...
#| warning: false
#| results: asis
#| tbl-cap: "Résultats du modèle de Cox non ajusté"
# ajustement du modèle de Cox non ajusté
cox_fit_non_ajuste <- coxph(surv_object_brut ~ rhc, data = df)
...
#| warning: false
#| results: asis
#| tbl-cap: 'Nombre de covariables avec |SMD| > 0,1 avant et après appariement (sans compter "distance")'
...
# création de l'objet de survie
surv_object_matched_mediane <- Surv(time = df_matched$survival_time_180d, event = df_matched$event_180d)
# ajustement du modèle de survie de Kaplan-Meier
km_fit <- survfit(surv_object_matched_mediane ~ rhc, data = df_matched)
# affichage du graphique de survie
km_plot <- ggsurvplot(
  km_fit,
  data = df_matched,
...
logrank_conventional <- survdiff(surv_object_matched_mediane ~ rhc, data = df_matched)
...
#| warning: false
#| results: asis
#| fig-cap: "Courbe de survie de Kaplan-Meier pour la première imputation appariée"
#| fig-width: 10
#| fig-height: 7.5
...
#| warning: false
#| results: asis
#création du tableau de synthèse des estimateurs
