...
-   On réalise l'appariement dans chaque dataset imputé, puis on combine les résultats des analyses réalisées dans chaque dataset imputeé en utilisant la règle de Rubin pour obtenir des estimations globales qui tiennent compte de l'incertitude liée à l'imputation.
...
df_matched_1 <- MatchIt::match.data(matchit_out_1)
...
df_matched_2 <- MatchIt::match.data(matchit_out_2)
...
df_matched_3 <- MatchIt::match.data(matchit_out_3)
...
df_matched_4 <- MatchIt::match.data(matchit_out_4)
...
df_matched_5 <- MatchIt::match.data(matchit_out_5)

# vérification des effectifs appariés
table(df_matched_1$rhc)
table(df_matched_2$rhc)
table(df_matched_3$rhc)
table(df_matched_4$rhc)
table(df_matched_5$rhc)
```
...
matchit_within <- MatchThem::matchthem(
  formula = formula_propension,
  datasets = imp_mids,
  approach = "within",
  method = "nearest",
...
df_matched_list <- MatchThem::complete(matchit_within, action = "all")
```
