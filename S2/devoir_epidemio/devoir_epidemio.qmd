...
-  Pour l'instant, ces valeurs "0" sont considérées comme des données manquantes et seront remplacées par `NA`.
...
-   Cette analyse n'est pas idéale car contribue à l'inflation du risque alpha par multiplication des tests.

-   En pratique, on peut simplement considérer que les patients ne sont par définition pas comparables, car il n'y ni randomisation, ni contrôle pour les facteurs de confusion potentiels.
...
#| warning: false
#| results: asis
#| tbl-cap: "Variables avec une différence moyenne standardisée (SMD) > 0.1 en valeur absolue"
...
-   Concernant la gestion des données manquantes, les auteurs disent avoir supposé que les données manquantes étaient des valeurs normales ("*Missing measurements were assumed to have normal values*"), ce qui correspond à une imputation simple (imputation par la valeur normale).
...
-   Une imputation est nécessaire avant appariement, car les méthodes d'appariement ne peuvent pas gérer les données manquantes. En effet, si une variable incluse dans le score de propension a des données manquantes, les patients avec des données manquantes pour cette variable seront exclus de l'appariement, ce qui peut réduire la taille de l'échantillon et introduire un biais si les données manquantes ne sont pas aléatoires.

### Méthode d'imputation 

-   Initialement : 

    -  Pour ce devoir, une imputation par la médiane avait initialement été effectuée pour les deux variables avec des données manquantes (score ADL et diurèse), ainsi que pour les autres variables avec données aberrantes.

    -  Ce choix était basé sur une recherche dans la littérature, qui suggérait que combinait imputation multiple et appariement était "tout sauf trivial", du fait de la gestion de la multiplicité des jeux de données imputés [@eisetConsiderationsUsingMultiple2022a].

-   Ensuite, après lectures spécialisées..

    -  Une imputation multiple a été réalisée pour les variables avec données manquantes

    -  Puis appariement et pooling des résultats à travers les jeux de données imputés.

-   De ce fait, j'ai choisi de laisser les deux méthodes dans ce devoir. Si je devais n'en choisir qu'une, je choisirais l'imputation multiple, qui est plus robuste que l'imputation par la médiane.

\newpage
## Partie 1 : imputation par la médiane et appariement selon score de propension

### Imputation par la médiane pour les variables avec données manquantes

-   Les variables numériques avec données manquantes ou aberrantes qui avaient été considérées comme manquantes sont imputées par la médiane de chaque variable.
...
### Calcul du score de propension et appariement après imputation par la médiane

-   Paramètrage du score de propension :

    -   Modèle de régression logistique avec comme variable dépendante la réalisation d'un cathéterisme (variable `rhc`) et comme variables indépendantes les variables sélectionnées (toutes les variables listées ci-dessus).

    -   Appariement 1:1 sans remplacement (c'est à dire que chaque patient ayant eu un cathéterisme est apparié à un seul patient n'ayant pas eu de cathéterisme, et qu'un patient n'ayant pas eu de cathéterisme ne peut être apparié qu'à un seul patient ayant eu un cathéterisme)
...
#| warning: false
#| results: asis
#| fig-cap: "Histogramme de densité du score de propension avant et après appariement"
#| fig-width: 10
#| fig-height: 4
...
-   Les distributions du score de propension sont similaires après appariement, ce qui suggère que l'appariement a réussi à équilibrer les groupes

-   Un histogramme miroir du score de propension permet de voir la même chose.
...
#| warning: false
#| results: asis
#| fig-cap: "Histogramme-miroir du score de propension après appariement"
#| fig-width: 10
#| fig-height: 3.5
...
-   Comme avant l'appariement, on peut visualiser l'équilibre des covariables basales après appariement à l'aide d'un love plot, qui montre les différences moyennes standardisées (SMD).

-   Après un appariement réussi, les SMD sont < 0,1 dans l'idéal. 
...
#| warning: false
#| results: asis
#| fig-cap: "Love plot visualisant les SMD avant-après appariement"
#| fig-width: 10
#| fig-height: 12
...
#| warning: false
#| results: asis
#| tbl-cap: 'Nombre de covariables avec SMD absolue > 0,1 avant et après appariement'
...
-   Ces tests ont tout de même été effectués en annexe.
...
## Partie 2 : imputation multiple et appariement selon score de propension

### Imputation multiple avec le package `mice`

#### Justification du choix de la méthode d'imputation

-   Dans l'analyse initiale, une imputation simple par la médiane avait été réalisée pour les variables avec des données manquantes, ce qui ne tient pas compte de l'incertitude liée aux données manquantes.

-   La méthode combinant imputation multiple et appariement est plus complexe et plusieurs articles ont été nécessaires pour la réalisation de ce devoir : 
    ...
    -   "Model Diagnostics for Remote Access Regression Servers" de Reiter, *Statistics and Computing* 2003 [@reiterModelDiagnosticsRemote2003]
...
-   Des données manquantes sont présentes dans les variables : `mean_blood_pressure`, `wbc`, `heart_rate`, `respiratory_rate`, `weight`, `adl_score` et `urine_output`.

-   Les variables de date et liées au décès ne sont pas inclues dans l'imputation, car elles ne sont pas des variables de prédiction pour le score de propension, mais plutôt des variables d'issue.
...
-   Les variables crées pour les analyses précédentes (variables de date, variables de résultat intermédiaire de survie, variables binaires pour les diagnostics et comorbidités) sont exclues des prédicteurs.
...
```{r}
#| label: multiple-imputation-annexe
#| echo: false
#| message: false
#| warning: false
#| results: hide
...
-   Le package `MatchThem()` est spécifiquement conçu pour apparier des données imputées multiples, et faciliter ensuite les analyses poolées des résultats à travers les jeux de données imputés.

-   Il prend comme entrée un objet `mids` (= objet d'imputation multiple "Multiple Imputed Data Set" créé par `mice()`).

```{r}
#| label: calcul-propension-within-annexe
#| echo: false
#| message: false
#| warning: false
#| results: hide
...
### Évaluation de la qualité de l'appariement après imputation multiple
...
```{r}
#| label: density-plot-propension-mi-annexe
#| echo: false
#| message: false
#| warning: false
#| results: asis
...
```{r}
#| label: histogram-mirror-propension-mi-annexe
#| echo: false
#| message: false
#| warning: false
#| results: asis
...
```{r}
#| label: love-plot-matched-mi-annexe
#| echo: false
#| message: false
#| warning: false
#| results: asis
#| fig-cap: "Love plot des SMD après appariement dans les données imputées multiples"
#| fig-width: 10
#| fig-height: 12
...
-   Elles sont toutes < 0,1.
\newpage

\newpage
# Analyse après appariement

::: {.callout-important title="Plan de l'analyse après appariement"}
-   Les résultats obtenus après imputation par la médiane sont présentés en première partie

-   Puis sont présentés les résultats obtenus après imputation multiple, qui sont ceux que je considère comme les plus robustes et sur lesquels je me baserais pour tirer des conclusions.
:::

## Dans la population appariée après imputation par la médiane

### Après imputation par la médiane : critères binaires (décès à 30 jours et à 180 jours)
...
### Après imputation par la médiane : régression logistique conditionnelle
...
### Après imputation par la médiane : survie
...
#### Prise en compte de la structure de dépendance introduite par l'appariement
...
#| warning: false
#| results: asis
#| tbl-cap: "Résultats du log-rank conventionnel et du log-rank stratifié sur les paires"
...
#| warning: false
#| results: asis
#| tbl-cap: "Résultats du modèle de Cox avec clustering sur les paires et estimation robuste de la variance"
...
#| warning: false
#| results: asis
#| fig-cap: "Graphique des résidus de Schoenfeld pour proportionalité des risques"
#| fig-width: 10
#| fig-height: 4
...
\newpage
## Dans la population appariée après imputation multiple
...
-   Effectifs des 5 jeux de données : 

```{r}
#| label: effectifs-apres-appariement-mi
#| echo: false
#| message: false
#| warning: false
#| results: asis
...
### Après imputation multiple : critères binaires (régressions logistiques conditionnelles) 
...
### Après imputation multiple : survie

### Kaplan Meier et log-rank
...
#### Après imputation multiple : modèle de Cox poolé avec clustering sur les paires
...
#| warning: false
#| results: asis
#| tbl-cap: "Modèle de Cox avec cluster sur les paires poolé sur les 5 imputations"
...
\newpage
# Autre approche : analyse multivariée sur population totale (non appariée)
...
-   On ne peut pas utiliser le jeu de données initiales avant imputation, car il contient des données manquantes sur les variables basales, ce qui rendrait l'ajustement du modèle de survie multivarié impossible ou biaisé.

-   En témoigne le tableau suivant qui simulerait l'effectif disponible pour le modèle de Cox multivarié si on utilisait les données brutes avant imputation, en incluant toutes les variables basales (84 variables) :

```{r}
#| label: effectif-cox-multivarie
#| echo: false
#| message: false
#| warning: false
#| results: asis
#| tbl-cap: "Effectif retenu en utilisant les données brutes avant imputation"
...
-   **Il faut donc utiliser les données après imputation multiple et ajuster le modèle de Cox dans chaque jeu de données imputé, puis pooler les résultats avec la règle de Rubin**

-   En annexe, sont présentées aussi : 

    -   les résultats d'un modèle de Cox multivarié ajusté sur les données brutes avant imputation (donc avec des données manquantes), à titre de comparaison. Ça diminue fortement l'effet car diminue énormément l'effectif, et ce n'est pas valide car > 10 événements par variable.

    -   les résultats d'un modèle de Cox multivarié ajusté sur données brutes en retirant ces variables basales avec des données manquantes, à titre de comparaison. Le HR obtenu est quasiment le même que celui du modèle de Cox multivarié poolé après imputation multiple.

### Modèle de Cox multivarié poolé après imputation multiple

-   Le modèle de Cox est ajusté dans chacun des jeux de données imputés, puis les estimateurs sont poolés avec la règle de Rubin.

-   Les variables basales incluses dans le modèle de Cox sont les mêmes que celles utilisées pour le score de propension, la variable d'événement est le décès à 180 jours. La variable d'exposition est le cathéterisme cardiaque droit (`rhc`).
...
```{r}
#| label: cox-multivariate-imputation-multiple
#| echo: false
#| message: false
#| warning: false
#| results: asis
#| tbl-cap: "Résultats du modèle de Cox multivarié poolé après imputation multiple"
...
-   Le risque instantané de décès à 180 jours calculé par analyse multivariée en population totale après imputation multiple  est de `r cox_multivariate_pooled_results$HR`, avec un intervalle de confiance à 95% de `r cox_multivariate_pooled_results$IC95`.

-   Le RHC est associé à un risque de décès à 180 jours significativement plus élevé que le non-RHC, même après ajustement pour les variables basales dans le modèle multivarié poolé sur les jeux imputés.

\newpage

# Synthèse des estimateurs et analyse de robustesse

-   Afin de vérifier la cohérence globale de nos résultats, on regroupe ci-dessous les estimateurs déjà calculés plus haut et on y ajoute le HR issu du modèle de Cox multivarié poolé après imputation multiple. Cela permet de comparer directement l'effet du RHC selon le critère étudié et la stratégie d'ajustement.

```{r}
#| label: tbl-comparaison-estimateurs
#| echo: false
#| message: false
#| warning: false
#| results: asis
...
-   Pour le décès à 30 jours, les estimations restent toutes supérieures à 1, avec des OR de `r or_summary_df_OR_30d_nonajuste`, `r or_summary_df_OR_30d_mediane_conditionnel` et `r or_summary_df_OR_30d_ajuste_multiple`, et des intervalles de confiance qui n'incluent pas 1. Cela suggère une augmentation du risque de décès à 30 jours chez les patients ayant eu un RHC.

-   Pour le décès à 180 jours, les résultats sont du même ordre de grandeur, avec des OR de `r or_summary_df_OR_180d_nonajuste`, `r or_summary_df_OR_180d_mediane_conditionnel` et `r or_summary_df_OR_180d_ajuste_multiple`. L'association défavorable observée à 30 jours persiste donc à 180 jours.

-   Pour la survie à 180 jours, les HR sont très proches entre l'analyse brute (`r cox_summary_non_ajuste_HR`), l'analyse appariée après imputation par la médiane (`r cox_summary_ajuste_mediane_HR`), l'analyse appariée après imputation multiple (`r cox_summary_ajuste_multiple_HR`) et l'analyse multivariée poolée après imputation multiple (`r cox_multivariate_pooled_results$HR`). Cette convergence renforce la robustesse du résultat, avec un excès de risque instantané de décès à 180 jours d'environ 20 à 30 % associé au RHC.

\newpage

# Au total : que conserver / que mettre dans l'analyse de sensibilité ?

-   Certaines analyses sont redondantes : analyse brute, imputations par la médiane et par imputation multiple, prise en compte de la structure de dépendance après appariement ou non...

-   **Pour la partie descriptive** : il n'y a pas de redondance : tout est à garder ! 

-   **Pour la partie analytique** : 

    - Imputation multiple + appariement

    - Estimation de l'OR conditionnel par paire pour les critères binaires

    - Log-rank avec prise en compte de l'appariement

    - Modèle de Cox avec prise en compte de l'appariement

-  En analyse de sensibilité / supplementaire :

    - L'analyse brute n'a pas vraiment d'intérêt car on sait que les groupes sont très différents, mais on peut la présenter pour montrer l'effet de l'ajustement.

    - L'analyse multivariée sur la population totale peut être présentée en analyse de sensibilité, pour montrer que les résultats sont similaires à ceux obtenus dans la population appariée, même sans appariement.

    - L'imputation par la médiane est une analyse de sensibilité intéressante pour montrer que les résultats sont globalement similaires, même avec une méthode d'imputation plus simple et plus conservatrice.

    - Les analyses sans prise en compte de l'appariement n'ont pas vraiment d'interêt, car elles ne tiennent pas compte de la structure de dépendance induite par l'appariement, et peuvent donc être biaisées (même si les résultats sont très proches).

\newpage
# Conclusion

-   Le cathéterisme cardiaque droit est associé à un risque de décès à 30 jours et à 180 jours significativement plus élevé que le non-RHC, que ce soit dans la population totale en analyse "brute", dans une population appariée sur un score de propension quelque soit la méthode d'imputation utilisée, ou dans une analyse multivariée ajustée pour les variables basales.
...
## Annexe 1 : tableau comparatif post-appariement
...
\newpage
## Annexe 2 : Analyse multivariée en population totale, sans imputation (données brutes)

### Sans imputation, avec toutes les variables basales (donc avec données manquantes)

-   Tableau de l'effectif disponible pour le modèle de Cox multivarié si on utilise les données brutes avant imputation, en incluant toutes les variables basales (84 variables) :

```{r}
#| label: effectif-cox-multivarie-avant-imputation
#| echo: false
#| message: false
#| warning: false
#| results: asis
...
#| warning: false
#| results: asis
#| tbl-cap: "Résultats du modèle de Cox multivarié, données brutes avec données manquantes"
...
-   L'exclusion des observations avec données manquantes dans le modèle de Cox multivarié (donc en utilisant les données brutes avant imputation) diminue fortement l'effectif disponible pour l'analyse, et le HR obtenu est de `r cox_multivariate_results$HR` avec un intervalle de confiance à 95% de `r cox_multivariate_results$IC95` n'est pas significatif.

-  Cependant, cette analyse n'est pas valide car elle ne respecte pas la règle de 10 événements par variable, et elle est très biaisée du fait de l'exclusion des patients avec données manquantes.

### Sans imputation, en retirant les variables basales avec des données manquantes

-   L'effectif total est utilisé car les variables avec données manquantes sont retirées du modèle.

```{r}
#| label: effectif-cox-multivarie-sans-missing
#| echo: false
#| message: false
#| warning: false
#| results: asis
...
```{r}
#| label: cox-multivariate-sans-missing
#| echo: false
#| message: false
#| warning: false
#| results: asis
#| tbl-cap: "Résultats du modèle de Cox multivarié, données brutes sans les variables avec données manquantes"
# identification des variables avec des données manquantes
variables_manquantes <- names(df_avant_imputation)[colSums(is.na(df_avant_imputation)) > 0]
# ajustement du modèle de Cox multivarié sans les variables avec des données manquantes
cox_multivariate_sans_missing_model <- coxph(
...
-   Finalement, une exclusion des variables avec des données manquantes dans le modèle de Cox multivarié ne change pas significativement les résultats, avec un HR de `r cox_multivariate_sans_missing_results$HR` et un intervalle de confiance à 95% de `r cox_multivariate_sans_missing_results$IC95`.
