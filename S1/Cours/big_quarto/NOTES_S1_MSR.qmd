...
-   Solutions :
    ...
    -   Approche simple : Modèles de régressions avec erreurs robustes : bootstrap en grappe, estimateurs sandwich

    -   Approche conditionnelle : Modèles linéaires mixtes (LMM) ou modèles linéaires généralisés mixtes (GLMM) : inclure un effet aléatoire pour le centre

    -   Approche marginale : Modèles marginaux (GEE) : corrige la moyenne marginale en appliquant une matrice variance-covariance basée sur la corrélation entre les observations au sein des centres 

## Approches classiques : modèles avec erreurs robustes
...
## Approches conditionnelles : modèles linéaires mixtes (LMM) et modèles linéaires généralisés mixtes (GLMM)

-   Inclure un effet aléatoire pour le centre dans le modèle de régression pour tenir compte de la corrélation entre les observations au sein d'un même centre

-   Permet de modéliser la variabilité entre les centres et de capturer la corrélation entre les observations au sein d'un même centre = corrélation intra-centre
...
-   Estimation des paramètres par maximum de vraisemblance (ML)ou maximum de vraisemblance restreint (REML)
...
## Approches marginales : modèle marginal = GEE (Generalized Estimating Equations)
...
-   Méthode : 
    ...
    -   La corrélation intra-centre est estimée à partir d'une matrice de corrélation des données.

    -   La moyenne marginale est modélisée via un modèle de régression en utilisant cette matrice de corrélation (donc "corrigée" pour la corrélation intra-centre
    
-   Par rapport à la méthode conditionnelle, on explore moins l'effet centre (pas d'effet aléatoire pour le centre donc pas d'estimation de la variance des effets aléatoires)
...
-   La méthode GEE est robuste aux non-normalités des données et aux valeurs aberrantes.
...
-   Même principe que pour les effets centres, mais ici c'est un effet sujet

-   Donc soit on applique une méthode classique de régression, soit une méthode conditionnelle ou marginale, soit un modèle de séries chronologiques.

### Approches classiques : modèles avec erreurs robustes

-   Idem que pour les effets centres, mais ici on applique une méthode classique de régression avec erreurs robustes.

-   Donc estimateur robuste de la variance (Sandwich estimator)

### Approches conditionnelles ou marginales : modèles linéaires mixtes et modèles marginaux (GEE)

-   Approche conditionnelle : modèles linéaires mixtes (LMM) ou modèles linéaires généralisés mixtes (GLMM)

    -   Inclure un effet aléatoire pour chaque individu dans le modèle de régression pour tenir compte de la corrélation entre les observations répétées pour un même individu

    -   Modéliser la variabilité entre les individus et de capturer la corrélation entre les observations répétées pour un même individu

    -   Estimation des paramètres par ML (bien pour le calcul des effets fixes) ou REML (bien pour le calcul des variances des effets aléatoires)

-   Approche marginale : modèles marginaux (GEE) 

    -   La prise en compte de la corrélation entre les observations répétées pour chaque individu est faite via une matrice de corrélation des données.

    -   La moyenne marginale est modélisée via un modèle de régression en utilisant cette matrice de corrélation.

    -   Par rapport à la méthode conditionnelle, on explore moins l'effet sujet (pas d'effet aléatoire pour le sujet donc pas d'estimation de la variance des effets aléatoires)

    -   Si peu de structure dans les données (faible corrélation intra-sujet), GEE se rapproche d'un modèle classique sans prise en compte de la corrélation.

## Modèles de spécifiques aux séries chronologiques = ARIMA (AutoRegressive Integrated Moving Average)
...
-   L'ordre du modèle AR est le nombre de périodes de temps qui sont corrélées entre elles.
...
Autocorrélogramme partiel fictif qui montre un processus AR1 et un processus MA1.

```{r}
#| fig-cap: "Autocorrélogramme partiel d'un processus AR(1) et Autocorrélogramme d'un processus MA(1)"
#| fig-width: 10
#| fig-height: 5
#| echo: false
#| message: false
...
pacf(ar1_data, main = "Autocorrélogramme partiel d'un processus AR(1) simulé (φ=0.7)")

# Simulation d'un processus MA(1)
ma1_data <- arima.sim(model = list(ma = 0.5), n = 100)
acf(ma1_data, main = "Autocorrélogramme d'un processus MA(1) simulé (θ=0.5)")
```
...
```{r}
#| fig-cap: "Exemple de diagramme des valeurs propres"
#| fig-height: 3.5
#| echo: false
#| results: 'asis'
#| include: true
#| message: false
#| warning: false
# Génération de données fictives avec structure factorielle claire
# pour créer un coude bien marqué dans le scree plot
set.seed(42)
n <- 150
p <- 6

# Création de 2 facteurs sous-jacents qui expliquent la majeure partie de la variance
facteur1 <- rnorm(n)
facteur2 <- rnorm(n)

# Génération des variables avec structure factorielle
# Les 3 premières variables sont fortement liées au facteur 1
# Les 3 suivantes sont fortement liées au facteur 2
donnees_fictives <- cbind(
  facteur1 * 2 + rnorm(n, sd=0.5),  # Item 1-3 liés au facteur 1
  facteur1 * 1.8 + rnorm(n, sd=0.5),
  facteur1 * 1.6 + rnorm(n, sd=0.5),
  facteur2 * 2 + rnorm(n, sd=0.5),  # Item 4-6 liés au facteur 2
  facteur2 * 1.8 + rnorm(n, sd=0.5),
  facteur2 * 1.6 + rnorm(n, sd=0.5)
)
colnames(donnees_fictives) <- paste0("Item", 1:6)

# Calcul de l'ACP
acp <- prcomp(donnees_fictives, scale. = TRUE)

# Valeurs propres = variance expliquée par chaque composante principale
valeurs_propres <- acp$sdev^2

# Scree plot
plot(valeurs_propres, type="b", pch=19, frame.plot=FALSE,
        xlab="Numéro de la composante", ylab="Valeur propre",
        main="Diagramme des valeurs propres (Scree plot)")
