...
Théorie qui dit que la multiplicité des tests statistiques augmente le risque d'erreurs de type I (faux positifs) : 

1.  En épidémiologie (statistique exploratoire : on s'en fiche un peu)

2.  On est dans Fisher en épidémiologie : notre but est de décrire, on a pas calculé le nombre de sujets nécessaires etc et on peut interpréter la p-value.

    -   Si pas de risque alpha, pas d'ajustement ! 